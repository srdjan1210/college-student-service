    		 ...
    		 for (int i = 0; i < students.size(); i++) {
    			 for(int j=0; j<failedSubjects.size();j++) {
    				 if(students.get(i).getFailedSubjects().contains(failedSubjects.get(j))) {
    					 if(writeNumber == 0) {
    						 System.out.println(students.get(i).getIndexNumber()+","+failedSubjects.get(j).getSubjectId());
    						 myWriter.write(students.get(i).getIndexNumber()+","+failedSubjects.get(j).getSubjectId());
    						 writeNumber++;
    					 }
    					 else {
    						 myWriter.append(students.get(i).getIndexNumber()+","+failedSubjects.get(j).getSubjectId());
    						 writeNumber++;
    					 }
