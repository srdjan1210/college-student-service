...
public class DeleteFailedSubjectController {
	...
	public DeleteFailedSubjectController(ToolbarEditStudentFailed editFailedPanel) {
		int resp = JOptionPane.showConfirmDialog(null, "Da li ste sigurni da zelite ukloniti nepolozeni predmet?", "Obrisi nepolozeni predmet?", JOptionPane.YES_NO_OPTION);
		if(resp == 1) return;
		String selectedSubjectId = editFailedPanel.getSelectedSubjectId();
		DataModel model = DataModel.getInstance();
		Boolean success = model.removeFailedSubjectFromStudentSubjects(selectedSubjectId, Screen.getInstance().getStudentTab().getSelectedStudentIndex());
		if(success) {
